...
bool showEmployees(const Employee* const* employees, int numEmployees)
{
	if (numEmployees == 0)
	{
...
bool showCustomers(const Customer* const* customers, int numCustomers)
{
	if (numCustomers == 0)
	{
		cout << "No Customers founded" << endl;
		return false;
	}
...
bool showProducts(const Product* const* products, int numProducts)
{
	if (numProducts == 0)
	{
		cout << "No Products founded" << endl;
		return false;
	}
...
void makeOrder(CoffeeShop& shop, Shift& shift)
{
	...
	if (showEmployees(shift.getEmployees(), shift.getNumEmployees()))
	{
		cin >> choice;
	...
	if (showCustomers(shop.getCustomers(), shop.getNumCustomers()))
	{
		cin >> choice;
	...
	if (!showProducts(shift.getDailyMenu(), shift.getDailyMenuSize()))
		return;
